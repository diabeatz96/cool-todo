...
const RegisterForm = () => {
    ...
    return (
        ...
        <div className="h-90v flex items-center justify-center">
            {response && (
            <div className={`${response.success ? "bg-green-200 broder-2 border-green-800 text-green-800" : 
            "bg-red-200 border-2 border-red-800 text-red-800"} py-2 px-5 my-10 text-center`}>
                <span className="font-bold">
                    {response.success ? `Success ${response.message ? `: ` : ``}` : "Failure: "}
                </span>
                {response.message}
            </div>
        )}
        ...
        <form className="form">
            <div className="form-control w-full max-w-xs">
                ...
                <input type="text" placeholder="name" name="name" className=" text-white input input-primary input-bordered w-full max-w-xs" 
                value={name} onChange={((e) => {dispatch({ type: e.target.name, value: e.target.value })})} />
            </div>
            <div className="form-control w-full max-w-xs">
                ...
                value={password} onChange={((e) => {dispatch({ type: e.target.name, value: e.target.value })})}  />
            ...
            <div className="form-control w-full max-w-xs">
                ...
                value={email} onChange={((e) => {dispatch({ type: e.target.name, value: e.target.value })})} />
            </div>
            {/* <div className="form-control w-full max-w-xs">
                <label htmlFor="email" className="label">
            ...
            <button className="btn mt-4 btn-primary w-full max-w-xs" onClick={register}>Register</button>
            </form>
            </div>
            </section>
            </div>
